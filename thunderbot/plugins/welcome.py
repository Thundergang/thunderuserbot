...
from thunderbot.plugins.sql_helper.welcome_sql import (
    add_welcome_setting,
...
@thunderbot.on(admin_cmd(pattern="savewelcome"))  # pylint:disable=E0602
@thunderbot.on(sudo_cmd(pattern="savewelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@thunderbot.on(admin_cmd(pattern="clearwelcome"))  # pylint:disable=E0602
@thunderbot.on(sudo_cmd(pattern="clearwelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
...
@thunderbot.on(admin_cmd(pattern="listwelcome"))  # pylint:disable=E0602
@thunderbot.on(sudo_cmd(pattern="listwelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
