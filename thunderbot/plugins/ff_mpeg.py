...
from thunderbot.utils import admin_cmd, progress

FF_MPEG_DOWN_LOAD_MEDIA_PATH = "uniborg.media.ffmpeg"


@thunderbot.on(admin_cmd(pattern="ffmpegsave"))
@thunderbot.on(sudo_cmd(pattern="ffmpegsave", allow_sudo=True))
...
@thunderbot.on(admin_cmd(pattern="ffmpegtrim"))
@thunderbot.on(sudo_cmd(pattern="ffmpegtrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
    ...
    if len(cmt) == 3:
        ...
        o = await take_screen_shot(
            FF_MPEG_DOWN_LOAD_MEDIA_PATH, TEMP_DOWNLOAD_DIRECTORY, start_time
        )
        logger.info(o)
        try:
            c_time = time.time()
