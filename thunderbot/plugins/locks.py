...
@thunderbot.on(admin_cmd(pattern="unlock ?(.*)"))
@thunderbot.on(sudo_cmd(pattern="unlock ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@thunderbot.on(admin_cmd(pattern="currenabledlocks"))
@thunderbot.on(sudo_cmd(pattern="currenabledlocks", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
...
@thunderbot.on(events.MessageEdited())  # pylint:disable=E0602
@thunderbot.on(events.NewMessage())  # pylint:disable=E0602
...
@thunderbot.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
            ...
            for user_id in added_users:
                ...
                if user_obj.bot:
                    ...
                    try:
                        ...
                        break
            if G_BAN_LOGGER_GROUP is not None and is_ban_able:
