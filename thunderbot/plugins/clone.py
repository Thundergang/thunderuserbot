...
from thunderbot import ALIVE_NAME, CMD_HELP, PRIVATE_GROUP_ID, TEMP_DOWNLOAD_DIRECTORY
from thunderbot.utils import admin_cmd

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "ThunderUserbot"
DEFAULTUSERBIO = "No bio found!"
...
@thunderbot.on(admin_cmd(pattern="clone ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@thunderbot.on(admin_cmd(pattern="revert$"))
async def _(event):
    if event.fwd_from:
        return
    name = f"{DEFAULTUSER}"
