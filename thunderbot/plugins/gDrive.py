...
from thunderbot import TEMP_DOWNLOAD_DIRECTORY, G_DRIVE_CLIENT_ID,G_DRIVE_CLIENT_SECRET,GDRIVE_FOLDER_ID, PRIVATE_GROUP_ID, AUTH_TOKEN_DATA
...
@thunderbot.on(admin_cmd(pattern="ugdrive ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@thunderbot.on(admin_cmd(pattern="drivesch ?(.*)"))
async def sch(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
@thunderbot.on(admin_cmd(pattern="gdrivedir ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
async def create_token_file(token_file, event):
    ...
    async with bot.conversation(int(PRIVATE_GROUP_ID)) as conv:
        await conv.send_message(
            f"Go to the following link in your browser: {authorize_url} and reply the code"
        )
        response = conv.wait_event(
            events.NewMessage(outgoing=True, chats=int(PRIVATE_GROUP_ID))
        )
...
@thunderbot.on(admin_cmd(pattern="gfolder ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
